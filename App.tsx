...
import { getPreLoginRoutes, getPostLoginRoutes, getCommonRoutes } from './src/utils/routes'

// Mocking some routes for navigation
const store = configureStore()
const Stack = createStackNavigator()
const theme = getProductTheme(false)

function App() {
  const [isLoading, setIsLoading] = useState(true)

  // Simulating a loading state (e.g., Firebase, auth)
  useEffect(() => {
    setTimeout(() => {
      setIsLoading(false)
        ...
        <SafeAreaProvider>
          <NavigationContainer>
            <Stack.Navigator initialRouteName="Landing">
              {getPreLoginRoutes(Stack)}
              {getPostLoginRoutes(Stack)}
              {getCommonRoutes(Stack)}
            </Stack.Navigator>
          </NavigationContainer>
        </SafeAreaProvider>
      </PaperProvider>
